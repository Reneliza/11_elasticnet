...
*any other position of the blue line, up or down, will not minimize the SSE
* goal of linear regression is to minimize SSE, that is, minimize the distance of all points to the predicted (blue line)
...
* L2 - sum of all the betas * lambda
...
* taking the absolute value of the betas unlike in ridge wherein the square of betas are taken
...
These packages are made available as a bundle through the meta-package `tidymodels` (https://www.tidymodels.org/packages/), and include:  
  ...
  - `parsnip` to specify model types and engines ; unify interface to models 
  - `tune` to fine-tune hyper-parameters  
  - `dials` to create grids  
  - `yardstick` to assess performance  ; assess metrics, RMSE
...
set.seed(931735) #random but reproducible
...
recipe(strength_gtex ~ .,
    ...
    step_rm(year, site,
            matches("Jan|Feb|Mar|Apr|Nov|Dec")) %>% #related to cotton growing season
...
ggplot(data = enet_grid,
  ...
  geom_point() # we're gonna be training models for each one of the points
```
...
enet_grid_result$.metrics[[1]] # 132 rows, 1 row for each combination
```
...
What penalty and mixture values created lowest RMSE?  The model with the lowest RMSE
...
R2 - increasing penalty, R2 is decreasing because the we are removing variable with increasing penalty
...
final_fit %>%
  collect_metrics()
```
11% of variability is explained by weather data
...
Metrics on train is better (lower RMSE, higher rsq) than the metrics on test because it as seen the dataset

provide the testing set metric
...
final_fit %>%
  ...
  ggplot(aes(x = strength_gtex,
             y = .pred)) +
  geom_point() +
  geom_abline() + #slope (0) and intercept(1) line 
  geom_smooth(method = "lm") +
  scale_x_continuous(limits = c(20, 40)) +
  scale_y_continuous(limits = c(20, 40)) 
```
* model does a not-so-good jobs in low and high strength values
...
```
* 37 predictors (started with 72), coefficient in est - 
* mean_dayl.s_Aug is most important
...
**Therefore, tmin, solar radiation (positive effect), and tmax (negative effect) in September were the most important variables affecting cotton fiber strength.**  -- Solar rad and t min in september are the two most important variables

Why is solar rad and t min most important variables in explaining cotton fiber quality
